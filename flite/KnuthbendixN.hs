module KnuthbendixN where
import Prelude()
import NanoPrelude

-- NanoPrelude port of Knuthbendix (was full-Prelude). Monomorphic Int throughout;
-- Char = Int so chr/ord are identity. Numeric literals default to Int (no Num/Integer). Answer 4792, 101,786 fun reductions (full-Prelude: 1.96M).
fromString :: [Int] -> [Int]
fromString x = x
chr :: Int -> Int
chr x = x
