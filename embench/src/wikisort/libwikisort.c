 ...
 * Upstream defines `bool` as a typedef which conflicts with the C99
 * builtin _Bool / <stdbool.h>. Newer gcc (C17/C23 default) treats
 * `bool` as a keyword. Use stdbool.h instead — same semantics, no
 * conflict. */
#include <stdbool.h>

/* Integer square root (Newton's method). Upstream wikisort calls
 * sqrt() which is libm + soft-double helpers (__fixdfsi, __adddf3,
 * ...) — we link freestanding without libgcc/libm, so this stand-in
 * keeps the same block-size formula without any FP. */
static long wiki_isqrt_l(long x)
{
  if (x <= 0) return 0;
  long r = x, y = 1;
...
#define sqrt(x) wiki_isqrt_l((long)(x))
