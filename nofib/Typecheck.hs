-- Typecheck.hs - nofib spectral/hartel/typecheck (Hartel suite: polymorphic
-- type checker).  THE BENCHMARK BODY IS THE MACHINE-TRANSLATED ORIGINAL,
-- REGENERATED VERBATIM from nofib's Main.hs, in the same explicit-brace block
-- upstream uses (`module ... where { ...; }`) -- its generated code splits
-- declarations across lines, which only parses that way.
--
-- This port takes the FULL Prelude, not the nano dialect.  Fast2haskell emits
-- a fixed set of float helpers into every translated program (f_abs, c_e,
-- c_pi, f_limit) built on negate/exp/atan at Float, and the nano dialect has
-- none of those.  A larger prelude costs nothing here and keeps the body
-- untouched.  Double -> Float is the usual rule: mhs's Double is FloatW, i.e.
-- binary32 on rv32, so Float is what makes both versions the same format.
--
-- The glue is the Fast2haskell shims this program actually reaches (fix,
-- miraseq through the real seq, pair) plus the port's hash.
-- SIM SCALE: 20 repetitions, not upstream's 500.  f_benchmark_main just
-- type-checks the same program list n times, so this is a linear trim: the
-- GHC version ran 5h07 / 245M cycles at 500 without finishing and the mhs version
-- exhausted a collector-free heap at 23.7M.  BOTH ARMS use 20.
-- Result: hash of the output string.
...
type
    ...
    f_abs::Float -> Float;
    f_abs a_x=
        if (((<=) :: (Float -> Float -> Bool)) a_x (0.00000 :: Float))
        then (((negate) :: (Float -> Float)) a_x)
        else
            a_x;
    f_and::[Bool] -> Bool;
    ...
    f_dropwhile a_f (a_a:a_x)=
        ...
        then (f_dropwhile a_f a_x)
            ...
            ((:) a_a a_x);
    c_e::Float;
    c_e=((exp) :: (Float -> Float)) (1.00000 :: Float);
    ...
    c_pi::Float;
    c_pi=((*) :: (Float -> Float -> Float)) (4.00000 :: Float) (((atan) :: (Float -> Float)) (1.00000 :: Float));
    f_postfix::t1 -> [t1] -> [t1];
