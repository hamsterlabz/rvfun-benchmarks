-- Sched.hs - nofib spectral/hartel/sched (Hartel benchmark suite:
-- job-shop schedule optimiser), NanoClasses dialect. THE BENCHMARK BODY
-- IS THE MACHINE-TRANSLATED ORIGINAL, UNTOUCHED apart from the wrapper,
-- exactly as the Wang port. Double maps to FloatW.
-- Input: 10 (FAST size, single pass). Result: hash of the output string.
module Sched where
import Prelude()
