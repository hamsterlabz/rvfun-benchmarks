-- Parser.hs - nofib real/parser (a Haskell parser, partain's), NanoPrelude
-- dialect. THE BENCHMARK BODY IS UPSTREAM'S, UNTOUCHED: every declaration
-- between this header and the input constant at the bottom is byte-for-byte
-- nofib real/parser/Main.hs. The port is this header plus the entry point.
--
-- It imports NanoClasses rather than NanoPrelude because the program derives
-- Eq and Show for its own token and syntax types and compares them; the
-- class-free NanoPrelude cannot express that. NanoClasses is NanoPrelude
-- plus Data.Eq and Text.Show, so String, Char and string literals are the
-- real ones here and the source needs no rewriting.
--
-- Below: the Prelude functions NanoPrelude does not carry, each the standard
-- definition, and the Data.Char predicates. Nothing here changes what the
-- benchmark computes.
module Parser where
import Prelude()
import NanoClasses
...
-- the FAST stdin (parser.faststdin, 992 bytes): the bare-metal target has
-- no file system, so getContents becomes this constant.
fastInput :: String
fastInput = "\n--==========================================================--\n--=== The parser.                                        ===--\n--===                                          Parser.hs ===--\n--==========================================================--\n\nmodule Parser where\n\n{- FIX THESE UP -}\n--utLookupDef env k def\n--   = head ( [ vv | (kk,vv) <- env, kk == k] ++ [def] )\npanic = error\n{- END FIXUPS -}\n\n--paLiteral :: Parser Literal\npaLiteral\n   = pgAlts\n     [\n        pgApply (LiteralInt.leStringToInt) (pgItem Lintlit),\n        pgApply (LiteralChar.head)         (pgItem Lcharlit),\n        pgApply LiteralString              (pgItem Lstringlit)\n     ]\n\npaExpr\n   = pgAlts\n     [\n        paCaseExpr,\n        paLetExpr,\n        paLamExpr,\n        paIfExpr,\n        paUnaryMinusExpr,\n        hsDoExpr []\n     ]\n\npaUnaryMinusExpr\n   = pgThen2\n        (\\minus (_, aexpr, _) ->\n             ExprApp (ExprApp (ExprVar \"-\") (ExprLiteral (LiteralInt 0))) aexpr)\n        paMinus\n        paAExpr\n"

-- upstream main is `print (hash (showx parser_res))` over stdin; here the
-- input is the constant above and bench is that same hash.
hash :: String -> Int
hash = foldl (\acc c -> ord c + acc*31) 0

bench :: Int
bench = hash (showx (parser_test (laMain fastInput)))
