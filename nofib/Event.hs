-- Event.hs - nofib spectral/hartel/event (Hartel benchmark suite:
-- nand-gate event-driven logic simulation), NanoClasses dialect. THE
-- BENCHMARK BODY IS THE MACHINE-TRANSLATED ORIGINAL, UNTOUCHED apart
-- from the wrapper, exactly as the Wang port. Double maps to FloatW.
-- Input: 1000 (FAST/10). Result: hash of the output string (port convention).
module Event where
import Prelude()
...
type
    ...
    f_cmp_d::FloatW -> FloatW -> Int;
    f_cmp_d a_x a_y=truncateD (entier ((-.) a_x a_y));
